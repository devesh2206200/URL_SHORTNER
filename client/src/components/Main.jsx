...
export default function Main({ token }) {
    ...
    return (
        <main>
            <Shortener addLink={addLink} token={token} />
            
            {links.length > 0 && (
                <div className="recent-links-section">
                    <h4 className="recent-title">Recent Shortened Links</h4>
                    <UrlList urlList={links} />
                    <div className="flex justify-center mt-5">
                        <TiDeleteOutline className="btn-cross" onClick={hideLinks} title="Clear Recent History" />
                    </div>
                </div>
            )}
